...
const OrganizerDashboard = ()=>{
    ...
    // Replace the fetchDashboardData function in your dashboard page.tsx
    const fetchDashboardData = async ()=>{
        try {
            ...
            // Use the existing analytics endpoint that works
            let actualTotalRevenue = 0;
            let actualTotalAttendees = 0;
            try {
                const revenueResponse = await fetch('http://localhost:5251/api/analytics/revenue-by-event?period=all', {
                    headers: {
                ...
                if (revenueResponse.ok) {
                    ...
                    actualTotalRevenue = revenueData.totalRevenue || 0;
                    actualTotalAttendees = revenueData.totalAttendees || 0;
                    console.log('✅ Successfully fetched revenue from analytics:', actualTotalRevenue);
                } else {
                    throw new Error(`Analytics API returned ${revenueResponse.status}`);
                }
            } catch (analyticsError) {
                console.warn('⚠️ Failed to fetch revenue from analytics, using event revenue as fallback:', analyticsError);
                // Fallback: calculate from events
                actualTotalRevenue = eventsResponse.reduce((sum, event)=>sum + (event.revenue || 0), 0);
                actualTotalAttendees = eventsResponse.reduce((sum, event)=>sum + (event.ticketsSold || 0), 0);
            }
            const now = new Date();
...
//# sourceMappingURL=_9f238668._.js.map